<?PHP
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST")
	{
		...
		{
			$query = "UPDATE benutzer SET id = 3 WHERE campusid = '$campusidSel'";
			
			if ($result = mysqli_query($connection, $query))
			{
				$err = "Erfolg! Der Moderator mit der campusid: $campusidSel wurde heraufgestuft!";
			}
			else 
			{
	...
	$query = "SELECT campusid FROM benutzer WHERE id = 1";
							...
							if ($_SESSION["id"] > 2)
							{
								...
								<LI class='nav-item dropdown'>
									...
									<DIV class='dropdown-menu' aria-labelledby='navbarDropdown'>
										...
										<A class='dropdown-item active' href='admin_moderator_hinzufuegen.php'>
				...
				<FORM action="<?PHP echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
					<SELECT class="form-control" name="campusid">
						...
						if ($i == 0)
						{
							echo 
							"
							<OPTION value=''>Keine campusid zum heraufstufen (es muss ein neuer Account sein!) vorhanden!</OPTION>;
							";
						}
						else 
						{

<?PHP
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST")
	{
		...
		$teilenummer		= trim($_POST["teilenummer"]);
		$link 				= trim($_POST["link"]);
		$beschreibung 		= trim($_POST["beschreibung"]);
		$stueck 			= trim($_POST["stueck"]);
		$foto 				= trim($_POST["foto"]);
		$eaglebib 			= trim($_POST["eaglebib"]);
		$lagerraum 			= trim($_POST["lagerraum"]);
		$lagerschrank 		= trim($_POST["lagerschrank"]);
		$meldebestand 		= trim($_POST["meldebestand"]);
		$rueckgabe_noetig	= trim($_POST["rueckgabe_noetig"]);
		/* /Variablen */
		
		if (empty($artikelName) || empty($teilenummer)
		|| empty($stueck))
		{
			$err = "ERROR: Der Artikelname, die Teilenummer und die Stueckzahl duerfen nicht leer sein!";
		}
		else 
		{
			$query = "INSERT INTO artikel (teilenummer, name, link, beschreibung, stueckzahl, stueckzahl_verfuegbar, foto, eaglebib, lager_raum, lager_schrank, meldebestand, rueckgabe_noetig) VALUES ('$teilenummer', '$artikelName', '$link', '$beschreibung', '$stueck', '$stueck', '$foto', '$eaglebib', '$lagerraum', '$lagerschrank', '$meldebestand', '$rueckgabe_noetig');";
		
			if ($result = mysqli_query($connection, $query))
			{
				$err = "Das hinzufuegen des Bauteils (teilenummer: $teilenummer) war erfolgreich.";
			}
			else 
			{
							...
							if ($_SESSION["id"] > 2)
							{
								...
								<LI class='nav-item dropdown'>
									...
									<DIV class='dropdown-menu' aria-labelledby='navbarDropdown'>
										<A class='dropdown-item active' href='admin_bau_hinzu.php'>
											Bauteil hinzufügen
			...
			<DIV class="center">
				...
				<FORM action="<?PHP echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
					...
					<DIV class="form-group">
						...
						<INPUT type="text" class="form-control" name="teilenummer">
					</DIV>
					<DIV class="form-group">
						<LABEL>
							Link:
					...
					<DIV class="form-group">
						...
						<TEXTAREA type="text" class="form-control" name="beschreibung" cols=30 rows=5></TEXTAREA>
					</DIV>
					<DIV class="form-group">
						<LABEL>
							Stueckzahl:
						</LABEL>
						<INPUT type="text" class="form-control" name="stueck">
					</DIV>
					<DIV class="form-group">
						<LABEL>
							Foto:
					...
					<DIV class="form-group">
						...
						<INPUT type="text" class="form-control" name="eaglebib">
					</DIV>
					<DIV class="form-group">
						<LABEL>
							Lagerraum:
						</LABEL>
						<INPUT type="text" class="form-control" name="lagerraum">
					</DIV>
					<DIV class="form-group">
						<LABEL>
							Lagerschrank:
						</LABEL>
						<INPUT type="text" class="form-control" name="lagerschrank">
					</DIV>
					<DIV class="form-group">
						<LABEL>
							Meldebestand (ab wann Sie benachrichtigt werden wollen):
						</LABEL>
						<INPUT type="text" class="form-control" name="meldebestand">
					</DIV>
					<DIV class="form-group">
						<LABEL>
							Rückgabe nötig:
						</LABEL>
						<SELECT class="form-control" name="rueckgabe_noetig">
